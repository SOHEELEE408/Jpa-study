...
* 경로 표현식(다시 듣기)
  ...
  - 연관 필드 : 연관 관계를 위한 필드(단일값 연관 필드 - 대상이 엔티티, 커렉션 값 연관 필드 - 대상이 컬렉션)
  - 묵시적 조인을 사용하지 말고, 명시적 조인(JOIN 명시)을 사용한다. ➡ 실제 쿼리를 예상하기 어렵다.
...
* 페치 조인
  ...
  - 페이징과 함께 쓸 수 없다.
  - 즉시 로딩을 필요한 순간만 잠깐 쓸 수 있다.
  - JPA의 DISTINCT를 이용해서 같은 식별자를 가진 엔티티를 중복으로 간주하고 제거할 수 있다. 
  - **페치 조인은 객체 그래프를 SQL 한 번에 조회하는 개념**
  - 연관된 것은 모두 가져온다.
##### 📌 페치 조인의 한계
  - 페치조인 대상에는 별칭x
  - 둘 이상의 컬렉션은 페치 조인 x
  - 컬렉션을 페치 조인하면 페이징 API 사용X
    - **@BatchSize(size = )** : 지연로딩으로 엔티티를 조회할 때 로딩되는 시점에 in 쿼리로 size 만큼 한 번에 가져온다. 
    - persistence.xml에 적용하여 글로벌 전략으로 가져갈 수 있다.
  - _**여러 테이블을 조인해서 엔티티와 전혀 다른 형태의 결과값을 가져와야 할 때는 페치 조인보다는 일반 조인을 사용하는 것이 좋다. ➡ 필요한 데이터들만 조회해서 DTO로 반환하는 것이 효과적**_
    1. 페치 조인을 써서 엔티티로 조회
    2. 페치 조인 > 애플리케이션에서 DTO로 세팅 > 반환
    3. JPQL을 처음 짤 때부터 new DTO로 세팅
* 다형성 쿼리
  - TYPE : 조회 대상을 특정 자식으로 한정
  - TREAT : 자바의 타입 캐스팅과 유사
    - 자식 엔티티가 가진 값을 가지고 부모 엔티티로 조회하면 자식 엔티티가 조회된다.
* 엔티티 직접 사용
  - COUNT() 같은 함수를 사용할 때 파라미터로 값이 아닌 엔티티를 넘겨도 알아서 PK 값이 매칭이 된다.
* Named 쿼리
  - 자주 쓰는 쿼리를 이름을 붙여서 미리 선언해 놓을 수 있다.
  - 정적 쿼리만 가능
  - 어노테이션으로 정의 OR XML에 정의(XML이 항상 우선권 가짐)
  - 애플리케이션 로딩 시점에 초기화 후 재사용
  - 애플리케이션 로딩 시점에 쿼리를 검증 ➡ 컴파일 단계에서 오류 찾을 수 있다.
* 벌크 연산
  - N+1 문제 해결할 수 있음
  - 주의할 점
    - 벌크 연산을 먼저 실행하거나
    - 벌크 연산 후 영속성 컨텍스트를 초기화해야 한다.
    - DB에만 반영되는 것이지, 영속성 컨텍스트의 값이 바뀌지는 않는다.
    - 